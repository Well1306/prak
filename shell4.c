    ...
    while((pid = waitpid(-1, &status, WNOHANG)) > 0) {
        ...
        if(fnd) {
            ...
            if(WIFEXITED(status))
                ...
                printf("\n[%d]+    pid = %d    killed by signal %d\n", i + 1, pid, WTERMSIG(status));
            else if(WIFSTOPPED(status))
                printf("\n[%d]+    pid = %d    stopped by signal  %d\n", i + 1, pid, WSTOPSIG(status));
            proc[i] = 0;
...
/*
        ...
        for(i = 0; i < numb_c; i++) {
            ...
            if(flag_or) nextcommand = !success;
            if(flag_and) nextcommand = success;
            if(seps[i]) {
                if(!strcmp(seps[i],"|")) {
                    conv = 1;
                    pipe(fd);
                ...
                if(!strcmp(seps[i],"&&")) flag_and = 1;
                else flag_and = 0;
                if(!strcmp(seps[i],"||")) flag_or = 1;
                else flag_or = 0;
